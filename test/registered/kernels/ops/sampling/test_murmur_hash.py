...
def _murmur_hash32_reference(seed, positions, col_indices):
    ...
    seed = seed.cpu().to(torch.uint64).reshape(-1, 1)
    positions = positions.cpu().to(torch.uint32).to(torch.uint64).reshape(-1, 1)
    col_indices = col_indices.cpu().to(torch.uint32).to(torch.uint64).reshape(1, -1)

    M32 = torch.tensor(0xFFFFFFFF, dtype=torch.uint64)
    C1 = torch.tensor(0xCC9E2D51, dtype=torch.uint64)
    C2 = torch.tensor(0x1B873593, dtype=torch.uint64)
    N5 = torch.tensor(5, dtype=torch.uint64)
    NN = torch.tensor(0xE6546B64, dtype=torch.uint64)
    ...
    def fmix(h):
        h = (h ^ (h >> 16)) & M32
        h = (h * torch.tensor(0x85EBCA6B, dtype=torch.uint64)) & M32
        h = (h ^ (h >> 13)) & M32
        h = (h * torch.tensor(0xC2B2AE35, dtype=torch.uint64)) & M32
        h = (h ^ (h >> 16)) & M32
        return h

    h = mix(torch.zeros_like(seed), seed & M32)
    h = mix(h, (seed >> 32) & M32)
    h = mix(h, positions)
    h = mix(h, col_indices)
    h = h ^ 16
    return fmix(h).to(torch.uint32).reshape(-1)
...
def test_murmur_hash_jit_matches_reference(n, m, pos_dtype, col_dtype):
    ...
    col_indices = torch.arange(m, device="cuda", dtype=col_dtype)
...
def test_murmur_hash_production_shapes():
    ...
    seed = torch.randint(0, 2**63, (8,), dtype=torch.uint64, device="cuda")
